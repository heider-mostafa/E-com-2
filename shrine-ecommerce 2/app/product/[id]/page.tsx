...
export default function ProductPage({ params }: { params: { id: string } }) {
  ...
  const handleAddToCart = (customProduct = null) => {
    const itemToAdd = customProduct || {
      id: product.id,
      name: product.name,
      price: product.price,
      quantity: quantity
    ...
    addToCart(itemToAdd);
    setCurrentStock(prevStock => Math.max(0, prevStock - itemToAdd.quantity)); 
    toast({
      title: "Added to cart",
      description: `${itemToAdd.quantity} ${itemToAdd.name} added to your cart.`,
    });
  }
          ...
          <section className="mt-24 mb-16" aria-labelledby="upsell-heading">
            ...
            <div className="grid grid-cols-1 gap-4">
              <article className="flex items-center justify-between bg-white rounded-lg overflow-hidden shadow-lg p-4 relative">
                ...
                <Button onClick={() => handleAddToCart({ id: product.id, name: `${product.name} (Buy 2, Get 1 Free)`, price: 19.99, quantity: 1 })}>
                  Add to Cart
                </Button>
              </article>
              <article className="flex items-center justify-between bg-white rounded-lg overflow-hidden shadow-lg p-4 relative">
                ...
                <Button onClick={() => handleAddToCart({ id: product.id, name: `${product.name} (Buy 3, Get 2 Free)`, price: 29.99, quantity: 1 })}>
                  Add to Cart
                </Button>
              </article>
            </div>
